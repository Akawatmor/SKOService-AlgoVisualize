...
function EnfaToDfaConverter() {
  ...
  const onConnect = useCallback(async (params: Connection) => {
    const { value: label } = await Swal.fire({ title: 'Transition symbol', input: 'text',
      inputLabel: 'e.g. 0, 1, a, ε', showCancelButton: true });
  ...
  const initComputation = async () => {
    ...
    curEdges.forEach(e => {
      ((e.label as string)||'').split(',').forEach(s => {
        const t = s.trim();
        if (t==='\u03b5'||t.toLowerCase()==='e') { return; }
        if (t) symbols.add(t);
      });
    });
    if (symbols.size===0 && curEdges.length===0) { setErrorMsg('No transitions found. Add edges first.'); return; }
    ...
    const rows: DFARow[] = powerSet.map(sub => {
      ...
      // epsilon-closure helper (inline)
      const epsClosure = (startIds: string[]): string[] => {
        const vis = new Set<string>(startIds); const stack = [...startIds];
        while (stack.length>0) { const c=stack.pop()!;
          curEdges.forEach(e=>{ const l=(e.label as string)||'';
            const isEps=l.split(',').map(s=>s.trim()).some(s=>s==='\u03b5'||s.toLowerCase()==='e');
            if (e.source===c&&isEps&&!vis.has(e.target)){vis.add(e.target);stack.push(e.target);}
          });
        }
        return [...vis].sort();
      };
      alpha.forEach(sym => {
        // move(sub, sym) then epsilon-closure
        ...
        const closure = epsClosure([...reach]);
        transitions[sym] = getKey(closure.sort());
      });
      // start subset uses epsilon-closure of {startNode}
      // (already handled: we replace getKey([startNode.id]) with eClosure below)
      return { key, label, prettyName: sub.length===0?'\u2205':`{${sub.join(', ')}}`,
        isStart, isAccept, transitions, isReachable: false, isProcessed: false };
    });
    // For eNFA: start subset = epsilon-closure({startNode})
    const computeEpsClosure = (startIds: string[]): string[] => {
      const vis = new Set<string>(startIds); const stack=[...startIds];
      while(stack.length>0){const c=stack.pop()!;
        curEdges.forEach(e=>{ const l=(e.label as string)||'';
          const isEps=l.split(',').map(s=>s.trim()).some(s=>s==='\u03b5'||s.toLowerCase()==='e');
          if(e.source===c&&isEps&&!vis.has(e.target)){vis.has(e.target)||(vis.add(e.target),stack.push(e.target));}
        });
      }
      return [...vis].sort();
    };
    const startSubsetFull = computeEpsClosure([startNode.id]);
    const startKey = getKey(startSubsetFull);
    const markedRows = rows.map(r => ({
      ...r,
      isStart: r.key === startKey,
      isReachable: r.key === startKey ? true : r.isReachable,
    }));
  ...
  const handleExport = () => {
    ...
    Object.assign(document.createElement('a'),{href:url,download:'dfa_from_enfa.json'}).click();
    ...
    <div className="flex h-screen flex-col bg-slate-950 text-slate-100 font-sans">
      <header className="flex items-center justify-between px-5 py-3 border-b border-slate-800 bg-slate-900/60 backdrop-blur shrink-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">ε-NFA → DFA Converter</h1>
            <p className="text-[11px] text-slate-500">Subset Construction with ε-closure — full 2ⁿ table</p>
          </div>
        </div>
        ...
        <div className="w-[28%] border-r border-slate-800 flex flex-col relative bg-slate-900/20">
          <div className="absolute top-3 left-3 z-10 pointer-events-auto">
            <div className="bg-slate-900/90 border border-slate-700 rounded-lg p-2.5 backdrop-blur shadow-xl">
              <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-1.5">Input ε-NFA</p>
              ...
              <p className="text-[10px] text-slate-600 mt-2 leading-relaxed">Dbl-click → accept<br/>Right-click → start<br/>Use ε or e for epsilon</p>
            </div>
          </div>
          <ReactFlowProvider>
...
export default function EnfaToDfaPage() { return <EnfaToDfaConverter />; }
