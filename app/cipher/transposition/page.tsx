...
import { ArrowLeft, Home, Shuffle, Grid3x3, Columns, Table, ChevronRight, Info } from 'lucide-react';
...
const transpositionCiphers: Cipher[] = [
  {
    id: 'railfence',
    name: 'Rail Fence Cipher',
    description: 'Text is written in a zigzag pattern across multiple rails, then read off row by row.',
    complexity: 'O(n)',
    security: 'Very Low',
    keyType: 'Number (rails)',
    example: {
      plaintext: 'HELLO WORLD',
      key: '3',
      ciphertext: 'HOREL LWLOD',
    },
    historical: {
      year: 'Ancient',
      usage: 'Simple military encryption',
    },
    color: 'cyan',
    icon: Shuffle,
    hasVisualizer: true,
  },
  {
    ...
    description: 'Plaintext is written in rows, then columns are rearranged according to a keyword.',
    complexity: 'O(n)',
    security: 'Low',
    ...
    example: {
      plaintext: 'HELLO WORLD',
      key: 'KEY',
      ciphertext: 'ELhlolordw',
    },
    historical: {
      year: '1800s',
      usage: 'Military and diplomatic communications',
    },
    color: 'blue',
    icon: Columns,
  },
  {
    id: 'scytale',
    name: 'Scytale Cipher',
    description: 'Ancient Greek cipher using a rod of specific diameter. Text written around the rod.',
    complexity: 'O(n)',
    security: 'Very Low',
    keyType: 'Number (diameter)',
    example: {
      plaintext: 'HELLO',
      key: '2',
      ciphertext: 'HLOEL',
    ...
    historical: {
      year: '~400 BC',
      inventor: 'Ancient Greece (Sparta)',
      usage: 'Spartan military communications',
    },
    color: 'purple',
    icon: Grid3x3,
  },
  {
    id: 'route',
    name: 'Route Cipher',
    description: 'Write text in a grid, then read off following a specific route pattern.',
    complexity: 'O(n)',
    security: 'Low',
    keyType: 'Route pattern',
    example: {
      plaintext: 'HELLO WORLD',
      key: 'Spiral',
  ...
  },
  {
    ...
    description: 'Variation of columnar transposition handling repeated letters in the key differently.',
    complexity: 'O(n)',
    security: 'Low',
    keyType: 'Text (Keyword)',
    example: {
      plaintext: 'HELLO WORLD',
      key: 'TOMATO',
      ciphertext: 'LOLWREDLOH',
    },
    historical: {
      year: '1902',
      inventor: 'Émile Victor Myszkowski',
      usage: 'Cryptographic research',
    },
    color: 'orange',
    icon: Columns,
  },
  {
    ...
    description: 'Apply columnar transposition twice for increased security.',
    complexity: 'O(n)',
    security: 'Medium',
    keyType: 'Two keywords',
    example: {
      plaintext: 'HELLO',
      key: 'KEY1, KEY2',
      ciphertext: 'OLLHE',
    ...
    historical: {
      year: 'WWI & WWII',
      usage: 'Military encryption during World Wars',
    },
    color: 'pink',
    icon: Shuffle,
  },
...
export default function TranspositionCiphersPage() {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden">
      ...
      <nav className="sticky top-0 z-20 border-b border-slate-800/80 bg-slate-950/80 backdrop-blur">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-3 flex items-center justify-between">
          <div className="text-sm text-slate-400">Transposition Ciphers</div>
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 md:px-8 py-8 md:py-12 flex flex-col gap-8 md:gap-12 relative z-10">
        ...
        <header className="text-center space-y-4">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight bg-gradient-to-r from-pink-400 via-orange-400 to-amber-400 bg-clip-text text-transparent pb-2 drop-shadow-[0_0_16px_rgba(236,72,153,0.25)]">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-100 mb-2">About Transposition Ciphers</h2>
              <p className="text-slate-400 text-sm leading-relaxed">
                Unlike substitution ciphers, transposition ciphers don't change the characters themselves - they only 
                rearrange their positions. The plaintext letters remain the same, but their order is scrambled according 
                to a systematic rule. These were commonly used in ancient and medieval cryptography.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {transpositionCiphers.map((cipher) => {
            ...
            return (
              <Link
                key={cipher.id}
                href={cipher.hasVisualizer ? `/cipher/transposition/${cipher.id}` : '#'}
                onClick={(e) => {
              ...
              >
                <div className="relative z-10 flex flex-col h-full gap-4">
                  ...
                  {/* Try Button */}
                  <div className="mt-auto flex items-center text-sm font-medium text-slate-500 group-hover:text-pink-400 transition-colors">
